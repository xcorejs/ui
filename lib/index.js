...
const colorTransform = (val, s) => {
  ...
  if (typeof val === 'string' && val.startsWith('__')) {
    ...
    if (reg) {
      ...
      const value = core.get(scale, color, lightColorTheme[color] || color);
...
const getWidth = (n, scale) => core.get(scale, n, !isNumber(n) || n > 1 ? n : n * 100 + '%');
...
const getMargin = (n, scale) => {
  ...
  const value = core.get(scale, absolute, absolute);
...
const selectionSystem = core.system({
  color: {
    property: 'color',
    scale: 'colors',
    transform: colorTransform
...
const boxSystem = core.system({ ...layoutConfig,
  ...spaceConfig,
  ...typographyConfig,
  ...visualsConfig,
  ...borderConfig,
...
const pseudoBoxSystem = core.system({
  content: true
});
const globalSystem = core.system({
  boxSizing: {
    property: 'boxSizing'
  }
});
const flexSystem = core.system({
  display: true
});
const textSystem = core.system({
  textTransform: true,
...
const pathSystem = core.system({
  fill: {
    property: 'fill',
    scale: 'colors'
  }
});
const pathHoverSystem = core.system({
  fillHover: {
    property: 'fill',
    scale: 'colors'
  }
...
const selectionBase = p => styled.css`
  ${selectionSystem(p)}
`;
const boxBase = p => styled.css`
  ${boxSystem(p)}
  ...
  ${Object.keys(pseudoSelectors).map(k => p[k] && styled.css`
      ${pseudoSelectors[k]} {
  ...
  ${p._group && Object.keys(groupPseudoSelectors).map(k => p._group[k] && styled.css`
...
const pseudoBoxBase = base([boxBase], p => styled.css`
  ${pseudoBoxSystem(p)}
`);
...
const iconBase = base([flexBase], p => styled.css`
  flex-shrink: 0;
...
const textBase = base([boxBase], p => styled.css`
  ${textSystem(p)}
`);
...
const Box = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${compose(boxBase)}
`;
...
const useMerge = (target, ...sources) => React.useMemo(() => merge(target, ...sources), [target, ...sources]);
...
const TextStyle = styled__default.span.withConfig({
  shouldForwardProp
})`
  ${composedTextBase}
`;
...
const ActiveBreakpoint = props => {
  ...
  }, "[0]: \"_\" (0 - ", breakpoints[0], ")"), breakpoints.aliases.map((a, i) => /*#__PURE__*/React__default.createElement(Box, {
    key: a,
    display: [...style.slice(0, i + 1), 'block', ...style.slice(i + 2)]
  }, "[", i + 1, "]: \"", a, "\" (", breakpoints[i], " - ", i === breakpoints.length - 1 ? '∞' : breakpoints[i + 1], ")"))));
};

const Flex = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${composedFlexBase}
`;
...
const AspectRatioStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${composedBoxBase}
...
const renderComponent = c => typeof c === 'function' ? /*#__PURE__*/React.createElement(c) : c;
...
const IconStyle = styled__default.span.withConfig({
  shouldForwardProp
})`
  ${composedIconBase}
`;
...
const Img = ({ ...props
}) => /*#__PURE__*/React__default.createElement(ImgStyle, Object.assign({}, props));
const imgSystem = core.system({
  objectFit: true
});
const ImgStyle = styled__default.img.withConfig({
  shouldForwardProp
})`
  ${composedBoxBase}
  ${imgSystem}
...
const LoremIpsum = /*#__PURE__*/React.forwardRef(({
  count,
  units,
  random,
  ...props
}, ref) => {
  const pseudoRandom = React.useMemo(() => pseudoRandomGenerator(), []);
  const text = React.useMemo(() => loremIpsum.loremIpsum({
    count,
...
const mediaQueries = (breakpoints, predicate) => breakpoints.map((br, i) => {
  ...
  return isValueEmpty(val) ? '' : i === 0 ? val : styled.css`
...
const GridStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${composedBoxBase}
  ...
  return !(columns.empty(i) && rows.empty(i) && gap.empty(i)) && styled.css`
        ${!(columns.empty(i) && gap.empty(i)) && styled.css`
          -ms-grid-columns: ${parseTemplate(colVal, gapVal ? gapVal[0] : null).join(' ')};
        `}

        ${!(rows.empty(i) && gap.empty(i)) && styled.css`
...
`;
const gridSystem = core.system(gridConfig);

const templateQueries = (columns, rows, gap) => styled.css`
  ${rows.map((_, y) => styled.css`
      ${columns.map((c, x) => styled.css`
...
const SimpleGrid = ({
  ...
  return /*#__PURE__*/React__default.createElement(Grid, Object.assign({
    ...
    rows: breakpoints.map((_, i) => {
      ...
      return !gapRow.empty(i) || !cols.empty(i) || !unit.empty(i) ? `repeat(${Math.ceil(React.Children.count(children) / cols.get(i))}, ${(_unit$get2 = unit.get(i)) !== null && _unit$get2 !== void 0 ? _unit$get2 : '1fr'})` : null;
    }),
    gap: _gap
  }, props), children);
};
...
`;
const SpinnerStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${composedBoxBase}
...
const Stack = ({
  ...
  const getStyle = i => {
    const last = React.Children.count(children) === i + 1;
    return {
  ...
  return /*#__PURE__*/React__default.createElement(Flex, Object.assign({
    ...
    flexWrap: wrap
  }, props), React.Children.map(children, (child, i) => wrapItems ? /*#__PURE__*/React__default.createElement(Box, Object.assign({}, getStyle(i)), child) : /*#__PURE__*/React.isValidElement(child) ? /*#__PURE__*/React.cloneElement(child, getStyle(i)) : child));
};
...
const CellStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${composedBoxBase}
...
mediaQueries(p.breakpoints, i => {
  ...
  (!column.empty(i) || !gap.empty(i)) && colStart && styled.css`
            -ms-grid-column: ${colStart} !important;
          `, (!column.empty(i) || !gap.empty(i)) && colEnd && styled.css`
            -ms-grid-column-span: ${colEnd} !important;
          `, (!row.empty(i) || !gap.empty(i)) && rowStart && styled.css`
            -ms-grid-row: ${rowStart} !important;
          `, (!row.empty(i) || !gap.empty(i)) && rowEnd && styled.css`
...
const OutsetBox = ({
  ...
  const [update] = useDebounce.useDebouncedCallback(up, 500);
  React.useLayoutEffect(() => {
    t && up();
  });
  React.useEffect(() => {
...
const Complement = ({
  ...
  icon,
  element
}) => /*#__PURE__*/React__default.createElement(React__default.Fragment, null, renderComponent(element), icon && /*#__PURE__*/React__default.createElement(Icon, Object.assign({
  mr: "1rem",
  svg: icon
}, _icon)));
...
const ButtonStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
...
const TagStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
...
const CloseControl = /*#__PURE__*/React.forwardRef((p, ref) => {
  const {
    closeControl
  } = useTheme();
  const {
...
const CloseButtonStyle = styled__default.div.withConfig({
  shouldForwardProp
})`
  ${composedFlexBase}
  svg {
...
const Container = /*#__PURE__*/React.forwardRef((p, ref) => {
  const {
    container
  } = useTheme();
...
const Link = /*#__PURE__*/React.forwardRef((p, ref) => {
  const {
    link
  } = useTheme();
...
const LinkStyle = styled__default.a.withConfig({
  shouldForwardProp
})`
  ${composedTextBase}
...
const List = /*#__PURE__*/React.forwardRef((p, ref) => {
  var _ref, _p$v;
...
const listSystem = core.system({
  counterReset: true
});
const itemSystem = core.system({
  paddingLeft: true,
  marginBottom: true,
  color: true,
  fontSize: true,
...
const bulletSystem = core.system({
  content: true,
  position: true,
  color: true,
  marginRight: true,
...
const ListStyle = styled__default.ul.withConfig({
  shouldForwardProp
})`
  ${composedTextBase}
...
const ListItemStyle = styled__default.li.withConfig({
  shouldForwardProp
})`
  ${compose(flexBase, textBase)}
`;
...
const TypographyStyle = styled__default.p.withConfig({
  shouldForwardProp
})`
  ${composedTextBase}
`;
...
const useModalHistory = () => React.useContext(ModalContext);
const ModalInstanceContext = /*#__PURE__*/React.createContext({
  hide: false
});
...
const Modal = ({
  ...
  const {
    hide
  } = React.useContext(ModalInstanceContext);
  const {
    pop
...
const ModalProvider = ({
  ...
  return /*#__PURE__*/React__default.createElement(ModalContext.Provider, {
    value: context
  }, children, position !== -1 && queue.map(([m, props], i) => /*#__PURE__*/React__default.createElement(ModalInstanceContext.Provider, {
    key: i,
    value: {
      hide: i !== position
    }
...
const makeAlias = v => /*#__PURE__*/React.forwardRef((p, ref) => /*#__PURE__*/React__default.createElement(Text, Object.assign({
  v: v
}, p, {
  ref: ref
})));
...
const makeAlias$1 = v => /*#__PURE__*/React.forwardRef((p, ref) => /*#__PURE__*/React__default.createElement(Typography, Object.assign({
  v: v
}, p, {
  ref: ref
})));
...
const useDisclosure = (initialState = false) => {
  const [isOpen, setState] = React.useState(initialState);
